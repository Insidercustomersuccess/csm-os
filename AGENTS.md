# Vault map

This is a private, lean Insider CSM Work OS. `AGENTS.md` and `CLAUDE.md` must remain identical.

## Read first

- Start with `README.md`.
- Read `System/connections.md` before accessing an external service or tracker.
- Read `CSM Scorecard.md` before working with OKRs, KPIs or customer-marketing targets.
- Use the matching file under `Templates/` before creating a record.
- Read `Notes/Principles of an Agent-Friendly Obsidian Vault.md` before changing the structure.

## Canonical ownership

- Organisation records own customer strategy, account context and battle plans.
- People records own stakeholder context.
- Projects own finite customer outcomes.
- Meetings preserve event evidence and decisions.
- The linked Google Sheet owns structured CSM scorecard values and calculations.
- Official company systems retain their declared commercial and operational facts.
- `Tasks.md` owns only shared or leadership commitments not already owned elsewhere.

## Hard rules

- Do not duplicate Google Sheet metrics in Markdown.
- Do not copy every CSM task into `Tasks.md`.
- Do not infer performance from stale, missing or invalid evidence; use `Not assessable`.
- Preserve existing content, links and uncertainty.
- Keep edits narrow and read changed files back.
- Never store secrets.
- Confirm before external sends, deletions, permission changes or writes to Sheets, CRM or another official system.
- Do not add folders, record types, dashboards or automation unless explicitly approved.
